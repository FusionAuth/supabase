...
If you don't have a FusionAuth instance already, go through the [Docker installation guide](https://fusionauth.io/docs/v1/tech/installation-guide/docker) to install FusionAuth in your machine. If you don't want to run it on Docker, [there are other options](https://fusionauth.io/docs/v1/tech/installation-guide/).

In the [administration interface](http://localhost:9011), browse to **Tenants** and click the blue edit icon on the default tenant. Set the **Issuer** to your FusionAuth instance domain (e.g. `auth.example.com`) and save it by clicking the blue floppy disk icon in the top right corner of the page.

Now, you'll create an application. In FusionAuth, an application is anything a user can log into: a custom web app, commercial off the shelf software, a native app, or an API.

- Browse to **Applications** and click the green `+` icon to create a new application.
- Name it `Supabase`.
- Select the **OAuth** tab and put `http://localhost:3000/api/auth/callback/fusionauth` in **Authorized redirect URLs** _(this redirect will be handled by your Next.js application, which is built below)_.
- Go to the **JWT** tab and click the **Enabled** switch.
- Change **Access token signing key** to `Auto generate a new key on save...`.
- Save the application.

When you are redirected back to the **Applications** page, locate the application you created and click the green magnifying glass to view its details. Find the **OAuth configuration** section and capture both the `Client Id` and `Client secret`. You'll use these in the Next.js application you'll build below.

Now, register your user account for the newly created `Supabase` application.

- Navigate to the **Users** tab and find your user.
- Click on the black **Manage User** button under the **Action** heading in the list.
- Copy the **User Id** as you'll need this value later.
- Click on **Add registration**.
    - If you only have one application, it will be automatically selected under **Application**. If you have more than one, you need to select it.
- Save your user.

After saving your user, you are on the page listing **Users**.

Create another user to demonstrate that different records are restricted to different users. 

To do so, click the green `*` icon to create a new user. Then repeat the registration steps above to register the new user account for the `Supabase` application.
...
Here's an example [Kickstart file](https://github.com/FusionAuth/fusionauth-example-supabase/blob/main/kickstart/kickstart.json) which sets up FusionAuth for this tutorial.
...
Create a Next.js application by running the command below.
...
Create folder `src/pages/api/auth` and a file named `[...nextauth].js`. You'll configure the [built-in support for FusionAuth](https://next-auth.js.org/providers/fusionauth). This will make every request to `/api/auth/*` handled by NextAuth.js.
...
Then, change your home component located in `src/pages/index.js` to include the `<LoginButton />` component anywhere you want in the fragment.
...
Your application will need to call the FusionAuth API to obtain a custom JWT with the required claims for Supabase authorization. The default JWT created by FusionAuth doesn't quite have the right claims. In particular, the `aud` claim in the FusionAuth JWT represents the FusionAuth application, but Supabase needs the `aud` claim to be the value `authenticated`.

The API for the custom JWT needs an API Key, which can be created by navigating to `Settings > API Keys` in your FusionAuth instance and clicking the green `+` icon. In the `Endpoints` table, enable `POST` for the `/api/jwt/vend` endpoint and save it by clicking the blue floppy disk icon.

![Creating new API Key](/docs/img/guides/integrations/fusionauth/api-key-vend-jwt.png)

<Admonition type="warning">
This API Key can be used to create an arbitrary JWT signed by FusionAuth, so you'll want to keep it safe. This example stores it in an environment variable, but you can use a secrets manager or other solution as well.
</Admonition>
...
1. Name it `events`.
2. Make sure **Enable Row Level Security (RLS)** is checked _(you'll learn more about RLS in the next step)_.
3. Then, add the fields for this table. For every row below, click `Add column`. Set the name. Then, click the cog icon to edit its properties and uncheck **Is Nullable**, since all of these fields are required.
    | Name     | Type          |
    |-----------|---------------|
    | `title`   | `text`        |
    | `date`    | `timestamptz` |
...
<Image src="/docs/img/guides/integrations/fusionauth/create-table.png" alt="Creating table" width={671} height={936} />

Now that the `events` table is created, click the green `Insert` button and select `Insert row` to create a new record.

- Leave the `id` empty.
- Fill something in the `title`, like `supabase conference`.
- Select any `date`.
- Paste the **User Id** you copied from your FusionAuth user in `user_id`.
- Click `Save`.

Repeat these steps to insert at least one more record. For the second record, use the Id of the second user you created. Feel free to create as many event records as you'd like!

<Image src="/docs/img/guides/integrations/fusionauth/insert-row.png" alt="Inserting row in Supabase" width={673} height={938} />
...
For this application, you'll extract a user Id from the JWT to determine which data to show. For FusionAuth, the user Id is stored in the `sub` claim of the JWT. Create an SQL function by clicking [SQL editor](https://app.supabase.com/project/_/sql) in the sidebar, pasting the contents below and hitting `RUN`.
...
## Step 8: Issuing JWTs with Supabase's secret

To issue a JWT that Supabase can trust, we need to import your project `JWT Secret` to your FusionAuth instance by logging into FusionAuth. Then browsing to `Settings > Key Master` and selecting `Import HMAC` in the top right button.
...
Update `src/pages/api/auth/[...nextauth].js` to add some [callbacks](https://next-auth.js.org/configuration/callbacks), which personalize your NextAuth.js session. We have provided some comments directly in code to better explain what is going on.
  ...
  callbacks: {
    async jwt({ token }) {
      if (!token.accessToken) {
         ...
         * As it must have the "aud" claim as "authenticated", we need to use
         * FusionAuth's Vend API to issue a JWT with this specific value.
         *
         ...
         try {
           const vending = await FusionAuthClient.vendJWT({
             claims: {
               ...
               // and will know to compare the JWT against any policies you've added to the requested resource (table or
               // row).
               aud: 'authenticated',
            },
            // You'll create this environment variable later
...
With these changes, the NextJS session has an `accessToken` property containing the JWT that Supabase needs.
...
Create a `src/utils/supabase.js` file to return a Supabase client by configuring your account URL, the anonymous public key and the JWT you generated in the previous step. These values will be stored in environment variables, which you'll configure below.
...
In that same file, the `Home` component check whether there are `events` being passed as a property and build a table with the records. To keep the code clean, create another function, called `renderTable()`, to actually render it.
...
As promised, in this section you will configure environment variables to pass secrets to your NextJS application.

<Admonition type="tip">
If you have cloned our [demo application](https://github.com/FusionAuth/fusionauth-example-supabase), duplicate the file `.env.local.dist`, name it `.env.local` and replace the values there.
</Admonition>
...
By implementing insert, update and delete operations, you can build a working event management application. FusionAuth is responsible for authenticating the user and you can manage them with the FusionAuth interface and APIs. You're also building on Supabase's RLS to ensure that users can only access their own events.
