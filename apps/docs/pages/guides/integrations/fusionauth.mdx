...
- Name it `Supabase`.
- Select the **OAuth** tab and put `http://localhost:3000/api/auth/callback/fusionauth` in **Authorized redirect URLs** _(this will be handled by your Next.js application soon)_.
...
You need to register your user for the created `Supabase` application.
...
Now that you are back to the **Users** page, create another user to demonstrate the different records being shown to different users. Click the green `*` icon to create a new user and repeat the steps above to register it for the application.
...
To make things simpler, we'll only show how to manually data using the web interface, but you can take a look [at these docs](https://supabase.com/docs/reference/javascript/insert) to see how to do that in your code.
...
You should repeat these steps and insert at least one more record for the other user you created previously.
...
<video width="99%" loop={true} muted={true} playsInline={true} controls={true}>
 ...
 Change `src/pages/api/auth/[...nextauth].js` to add some [callbacks](https://next-auth.js.org/configuration/callbacks), which are ways of personalizing your NextAuth.js session. We have provided some comments directly in code to better explain what is going on.
  ...
  callbacks: {
    async jwt({ token }) {
      if (!token.accessToken) {
         ...
         * FusionAuth's Vending API to issue a JWT with this specific value.
         *
