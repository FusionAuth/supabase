...
Now, you'll create an application.
...
After saving your user, you are on the **Users** list page. Create another user to demonstrate that different records are restricted to different users. Click the green `*` icon to create a new user and repeat the steps above to register the account for the Supabase application.
...
Then, change your home component located in `src/pages/index.js` to include the `<LoginButton />` component anywhere in the fragment.
...
Your application will need to call the FusionAuth API to obtain a custom JWT with the needed claims for Supabase authorization. This request demands an API Key, which can be created by navigating to `Settings > API Keys` in your FusionAuth instance and clicking the green `+` icon. In the `Endpoints` table, enable `POST` for the `/api/jwt/vend` endpoint and save it by clicking the blue floppy disk icon.
...
Repeat these steps to insert at least one more record. For the second record, use the Id of the second user you created.
